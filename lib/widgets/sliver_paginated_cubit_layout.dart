...
class SliverPaginatedCubitLayout<TData, TItem> extends StatelessWidget {
  /// Creates a PaginatedCubitLayout.
  const SliverPaginatedCubitLayout({
    super.key,
    required this.cubit,
...
class _SliverPaginatedLayoutList<TData, TItem> extends StatelessWidget {
  const _SliverPaginatedLayoutList({
    required this.state,
    required this.itemBuilder,
    this.separatorBuilder,
  ...
  Widget build(BuildContext context) {
    ...
    if (items.isEmpty) {
      return emptyState;
    } else if (separatorBuilder case final separatorBuilder?) {
